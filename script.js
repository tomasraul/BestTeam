...
botao.addEventListener('click', function(){
    ...
    if(comprimento.value.length > 0 && largura.value.length > 0){
        ...
        if(molho >= 1){
            resposta.innerHTML += `Equivalentes a ${molho} molho(s) `
        }
        let molho2 = resultado(comp, larg)[2] % 12
        if(molho >= 1 && molho2 > 0){
            resposta.innerHTML += `e mais ${molho2} barrotes <br>`
        }

        if(qr.checked){
            if (comp > 400) {
                let faltaComprimento = comp - 400
                resposta.innerHTML += `Comprimento: ${comp/100}m; faltará ${resultado(comp, larg)[1]} pedaços de ${(faltaComprimento/100).toFixed(2)}m cada <br>`
            }
            if (comp < 400) {
                let restoComprimento = 400 - comp
                resposta.innerHTML += `Comprimento: ${comp / 100}m; restará ${resultado(comp, larg)[1]} pedaços de ${(restoComprimento/100).toFixed(2)}m cada <br>`
            }
            if (larg > 400) {
                let faltaLargura = larg - 400
                resposta.innerHTML += `Largura: ${larg / 100}m; faltará ${resultado(comp, larg)[0]} pedaços de ${(faltaLargura/100).toFixed(2)}m cada <br>`
            }
            if (larg < 400) {
                let restoLargura = 400 - larg
                resposta.innerHTML += `Largura: ${larg / 100}m; restará ${resultado(comp, larg)[0]} pedaços de ${(restoLargura/100).toFixed(2)}m cada <br>`
            }
        }
        ...
        chapas.innerHTML += `-Area: ${areaChapas}m<sup>2</sup>; <br> -Chapas Necessarias ${numChapas}, `
        let restchapas = (areaChapas % 3.6).toFixed(2)
        if(restchapas > 0){
        chapas.innerHTML += `e resta uma chapa de ${restchapas}m<sup>2</sup>`
        }
